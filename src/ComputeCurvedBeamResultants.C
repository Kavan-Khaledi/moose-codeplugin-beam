...
#include "ComputeCurvedBeamResultants.h"

registerMooseObject(MOOSEAPPNAME, ComputeCurvedBeamResultants);

InputParameters
ComputeCurvedBeamResultants::validParams()
{
...
ComputeCurvedBeamResultants::ComputeCurvedBeamResultants(const InputParameters & parameters)
  : Material(parameters),
    ...
    _beam_force(declareProperty<RealVectorValue>("beam_forces")),
    _beam_moment(declareProperty<RealVectorValue>("beam_moments")),
...
void
ComputeCurvedBeamResultants::initQpStatefulProperties()
{
  _force[_qp].zero();
  _moment[_qp].zero();
  _beam_force[_qp].zero();
  _beam_moment[_qp].zero();
}

void
ComputeCurvedBeamResultants::computeQpProperties()
{
