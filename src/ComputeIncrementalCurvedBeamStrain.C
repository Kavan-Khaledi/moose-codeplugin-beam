...
ComputeIncrementalCurvedBeamStrain::ComputeIncrementalCurvedBeamStrain(
    ...
    _has_y_vector(isParamValid("y_orientation"))
{
  ...
  if (_ndisp != _nrot)
    mooseError(
        "ComputeIncrementalCurvedBeamStrain: The number of variables supplied in 'displacements' "
  ...
  // {
  //   _disp_eigenstrain[i] = &getMaterialProperty<RealVectorValue>("disp_" +
  //   _eigenstrain_names[i]); _rot_eigenstrain[i] = &getMaterialProperty<RealVectorValue>("rot_" +
  //   _eigenstrain_names[i]); _disp_eigenstrain_old[i] =
...
ComputeIncrementalCurvedBeamStrain::initQpStatefulProperties()
{
  ...
  const std::vector<RealGradient> * orientation =
      &_subproblem.assembly(_tid, _nonlinear_sys.number()).getFE(fe_type, 1)->get_dxyzdxi();
...
ComputeIncrementalCurvedBeamStrain::computeQpStrain()
{
  ...
  RealVectorValue grad_disp_0 = {
      (((2.0 * xi - 1) * _disp0(0) + (2.0 * xi + 1) * _disp1(0) + (-4.0 * xi) * _disp2(0))) /
          _original_length[0],
      (((2.0 * xi - 1) * _disp0(1) + (2.0 * xi + 1) * _disp1(1) + (-4.0 * xi) * _disp2(1))) /
          _original_length[0],
      (((2.0 * xi - 1) * _disp0(2) + (2.0 * xi + 1) * _disp1(2) + (-4.0 * xi) * _disp2(2))) /
          _original_length[0]};
  RealVectorValue grad_rot_0 = {
      (((2.0 * xi - 1) * _rot0(0) + (2.0 * xi + 1) * _rot1(0) + (-4.0 * xi) * _rot2(0))) /
          _original_length[0],
      (((2.0 * xi - 1) * _rot0(1) + (2.0 * xi + 1) * _rot1(1) + (-4.0 * xi) * _rot2(1))) /
          _original_length[0],
      (((2.0 * xi - 1) * _rot0(2) + (2.0 * xi + 1) * _rot1(2) + (-4.0 * xi) * _rot2(2))) /
          _original_length[0]};
  RealVectorValue avg_rot = {(h0 * _rot0(0) + h1 * _rot1(0) + h2 * _rot2(0)),
                             (h0 * _rot0(1) + h1 * _rot1(1) + h2 * _rot2(1)),
                             (h0 * _rot0(2) + h1 * _rot1(2) + h2 * _rot2(2))};
...
ComputeIncrementalCurvedBeamStrain::computeStiffnessMatrix()
{
  ...
  K11_local(1, 1) = (1.0 / 20) * A * shear_modulus * _original_length[0] +
                    youngs_modulus * Iz / _original_length[0] +
                    (1.0 / 12) * A * shear_modulus * _original_length[0] +
                    (4.0 / 3.0) * youngs_modulus * Iz / _original_length[0];
  K11_local(2, 0) = (-0.5) * shear_modulus * Az;
  K11_local(2, 2) = (1.0 / 20) * A * shear_modulus * _original_length[0] +
                    youngs_modulus * Iy / _original_length[0] +
                    (1.0 / 12) * A * shear_modulus * _original_length[0] +
                    (4.0 / 3.0) * youngs_modulus * Iy / _original_length[0];
  ...
  K13_local(1, 1) = (1.0 / 20) * A * shear_modulus * _original_length[0] -
                    youngs_modulus * Iz / _original_length[0] -
                    (1.0 / 12) * A * shear_modulus * _original_length[0] +
                    (4.0 / 3.0) * youngs_modulus * Iz / _original_length[0];
  K13_local(2, 0) = (-1.0 / 6.0) * shear_modulus * Az;
  K13_local(2, 2) = (1.0 / 20) * A * shear_modulus * _original_length[0] -
                    youngs_modulus * Iy / _original_length[0] -
                    (1.0 / 12) * A * shear_modulus * _original_length[0] +
                    (4.0 / 3.0) * youngs_modulus * Iy / _original_length[0];
  ...
  K15_local(1, 1) = (-1.0 / 10) * A * shear_modulus * _original_length[0] +
                    (1.0 / 6.0) * A * shear_modulus * _original_length[0] +
                    (-8.0 / 3.0) * youngs_modulus * Iz / _original_length[0];
  K15_local(2, 0) = (2.0 / 3.0) * shear_modulus * Az;
  K15_local(2, 2) = (-1.0 / 10) * A * shear_modulus * _original_length[0] +
                    (1.0 / 6.0) * A * shear_modulus * _original_length[0] +
                    (-8.0 / 3.0) * youngs_modulus * Iy / _original_length[0];
  ...
  K55_local(1, 1) = (6.0 / 5.0) * A * shear_modulus * _original_length[0] -
                    (2.0 / 3.0) * A * shear_modulus * _original_length[0] +
                    (16.0 / 3.0) * youngs_modulus * Iz / _original_length[0];
  K55_local(2, 2) = (6.0 / 5.0) * A * shear_modulus * _original_length[0] -
                    (2.0 / 3.0) * A * shear_modulus * _original_length[0] +
                    (16.0 / 3.0) * youngs_modulus * Iy / _original_length[0];
